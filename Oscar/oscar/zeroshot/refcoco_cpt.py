...
from oscar.datasets.refcoco_zsl_cpt_dataset import ZSLColorFinetuneDataset
...
def build_test_dataset(data_file, tokenizer, args):
    ...
    return ZSLColorFinetuneDataset(data_file, tokenizer=tokenizer, args=args)
...
def val(args, data_loader, model, tokenizer):
    ...
    for step, (img_keys, batch) in tqdm(enumerate(data_loader)):
        ...
        for i, (imgk, color_list, rect_list) in enumerate(zip(img_keys, colors, rects)):
            ...
            for j, (cur_color_set, cur_rect_set) in enumerate(zip(color_list, rect_list)):
                ...
                collected_scores.append(cur_color_scores[0:-1])
            # pass
            ...
            # to_save_predictions
            to_save_predictions[imgk] = {}
            to_save_predictions[imgk]["rects"] = collected_rects
            to_save_predictions[imgk]["scores"] = collected_scores.tolist()
            to_save_predictions[imgk]["max_idx"] = int(max_idx.cpu())
    ...
    pred_list = all_gather(to_save_predictions)
    for cur_preds in pred_list:
        for k, v in cur_preds.items():
            to_save_predictions[k] = v

    miou = 0
    if is_main_process():
        raw_gts = data_loader.dataset.anns_dic
        gts = {k: v['bbox'] for k, v in raw_gts.items()}
        for k, p in predictions.items():
            assert p[2] > p[0] and p[3] > p[1]
            p = [p[0], p[1], p[2] - p[0] + 1, p[3] - p[1] + 1]
            iou = computeIoU(p, gts[k])
            miou += iou > 0.5
            to_save_predictions[k]["correct"] = iou > 0.5
            to_save_predictions[k]["file_name"] = raw_gts[k]["file_name"]
            to_save_predictions[k]["caption"] = raw_gts[k]["caption"]
            to_save_predictions[k]["bbox"] = raw_gts[k]["bbox"]
        logger.info("miou: {:.2f}".format(miou / len(predictions) * 100))

        # save
        # to_save_predictions[k] = {"rects": None, "scores": None, "max_idx": None,
        #                           "correct": None, "file_name": None,
        #                           "caption": None, "bbox": None}
        # name = data_loader.dataset.datasest_name
        # json.dump(to_save_predictions, open("tmp/rst/zsl_color_rst/{}.json".format(name), "w"))
...
def main():
    ...
    if "refcocog" in args.test_dir:
        test_loaders = [
            make_data_loader(args, op.join(args.test_dir+"test", "predictions.tsv"), tokenizer, is_distributed=args.distributed, is_train=False),
            make_data_loader(args, op.join(args.test_dir+"val", "predictions.tsv"), tokenizer, is_distributed=args.distributed, is_train=False)
        ]
    else:
        test_loaders = [
            make_data_loader(args, op.join(args.test_dir + "val", "predictions.tsv"), tokenizer,
                             is_distributed=args.distributed, is_train=False),
            make_data_loader(args, op.join(args.test_dir + "testA", "predictions.tsv"), tokenizer,
                             is_distributed=args.distributed, is_train=False),
            make_data_loader(args, op.join(args.test_dir + "testB", "predictions.tsv"), tokenizer,
                             is_distributed=args.distributed, is_train=False),
        ]

    acc = []
    for test_loader in test_loaders:
        acc.append( val(args, test_loader, model, tokenizer) )
    logger.info("acc: {:.2f}. {}".format(np.mean(acc), acc))
