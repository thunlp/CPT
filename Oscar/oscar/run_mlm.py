...
from oscar.datasets.mlm_dataset import MLMTSVDataset
import pickle
import json
...
def build_dataset(data_file, tokenizer, args):
    ...
    return MLMTSVDataset(data_file, tokenizer=tokenizer, args=args)
...
def main():
    ...
    data_loader = make_data_loader(args, "predictions.tsv", tokenizer, is_distributed=False)
    dic = {}
    color_names = json.load(open("tmp/colors.json"))
    color_idxs = tokenizer.convert_tokens_to_ids(color_names)
    # color_idxs = [c[1] for c in colors]
    # color_names = [c[0] for c in colors]
    for step, (img_keys, batch) in tqdm(enumerate(data_loader), total=len(data_loader)):
        batch = tuple(t.to(args.device) for t in batch)
        im_feats, input_ids, input_mask, segment_ids, mask_pos = batch
        with torch.no_grad():
            x = model(input_ids, segment_ids, input_mask, img_feats=im_feats)
            scores = (x[0][:, mask_pos[0]])
            for im_key, sc in zip(img_keys, scores):
                sc = sc[color_idxs].softmax(0)
                vals, idxs = sc.sort(descending=True)
                vals = vals.tolist()
                lbs = [color_names[x] for x in idxs]
                dic[im_key] = list(zip(lbs, vals))
    pickle.dump(dic, open("tmp/3z.pk", "wb"))
