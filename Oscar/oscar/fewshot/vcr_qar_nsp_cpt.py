...
from oscar.modeling.modeling_vcr import VCRQAR_NSPCPT
...
class VCRDataset(Dataset):
    ...
    def __init__(self, args, name, data_file, tokenizer, label_pos_feats=None):
        ...
        examples = _load_dataset(args, name)
        # self.examples = [x for x in examples if x.img_key in self.imgid2feat]
        examples = {x.img_key: x for x in examples}
        self.examples = []
        for imgkey, featid in self.imgid2feat.items():
            sample = examples[imgkey.split("_")[0]]
            sample.img_key = imgkey
    ...
    def tensorize_example(self, example, cls_token_at_end=False, pad_on_left=False,
        ...
        ret_q_a = []
        ret_qa_r = []
        # text_b: [answers, rationales]
        for c in example.text_b[0]:
            ans_label, rat_label = _decode_label(example.label)
            ...
            fed_in_example.label = ans_label
            ...
            ret_q_a.append(rst)
        for c in example.text_b[1]:
            ans_label, rat_label = _decode_label(example.label)
            fed_in_example = copy.deepcopy(example)
            a = fed_in_example.text_b[0][ans_label]
            fed_in_example.text_a = self._vcr_textize(fed_in_example.text_a+a, colors, names)
            fed_in_example.text_b = self._vcr_textize(c, colors, names, colorful=True)
            fed_in_example.label = rat_label
            rst = self._tensorize(fed_in_example, img_feat_tuple, cls_token_at_end, pad_on_left,
                                  cls_token, sep_token, pad_token,
                                  sequence_a_segment_id, sequence_b_segment_id,
                                  cls_token_segment_id, pad_token_segment_id, mask_padding_with_zero)
            ret_qa_r.append(rst)
        return ret_q_a, ret_qa_r
...
def vcr_collate_fn(raw_batch):
    """
    :param batch: [b0, b1, b2, ... bn]  bi:[batch_q_a, batch_qa_r]
        batch_q_a: [q_a0, q_a1, q_a2, q_a3]  batch_qa_r: [qa_r0, qa_r1, qa_r3, qa_r4]
    :return:
    """
    def _raw_batch(batch):
    ...
    batch_q_a = [x[0] for x in raw_batch]
    batch_q_a, n_ans = _raw_batch(batch_q_a)

    batch_qa_r = [x[1] for x in raw_batch]
    batch_qa_r, n_rats = _raw_batch(batch_qa_r)
    return batch_q_a, n_ans, batch_qa_r, n_rats
...
def train(args, train_dataset, eval_dataset, model, tokenizer):
    ...
    logger.info("accumulation_steps: {}  n_gpu: {}".format(args.gradient_accumulation_steps, args.n_gpu))
    ...
    #for epoch in train_iterator:
        ...
        t_start = time.time()
        for step, (batch_q_a, n_ans, batch_qa_r, n_rats) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            model.train()
            q_a_input_ids, q_a_input_mask, q_a_segment_ids, q_a_img_feats = tuple(t.to(args.device) for t in batch_q_a[:-3])
            q_a_q_ids, q_a_labels = batch_q_a[-3], batch_q_a[-2]
            q_a_cls_labels = torch.ones([len(q_a_labels)], dtype=q_a_input_ids.dtype, device=q_a_input_ids.device)
            for i, lb in enumerate(q_a_labels[::n_ans]):
                q_a_cls_labels[i * n_ans + lb] = 0

            qa_r_input_ids, qa_r_input_mask, qa_r_segment_ids, qa_r_img_feats = tuple(
            ...
            q_a_inputs = {'input_ids':      q_a_input_ids,
                      'attention_mask': q_a_input_mask,
                      'token_type_ids': q_a_segment_ids if args.model_type in ['bert', 'xlnet'] else None,  # XLM don't use segment_ids
                      'next_sentence_label':         q_a_cls_labels,
                      'img_feats':      None if args.img_feature_dim == -1 else q_a_img_feats,
                      "head": "ans"}
            q_a_outputs = model(**q_a_inputs)
            q_a_loss, q_a_logits = q_a_outputs[:2]

            qa_r_inputs = {'input_ids': qa_r_input_ids,
                          'attention_mask': qa_r_input_mask,
                          'token_type_ids': qa_r_segment_ids if args.model_type in ['bert', 'xlnet'] else None,
...
def evaluate(args, model, eval_dataset=None, prefix="", tokenizer=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch_q_a, n_ans, batch_qa_r, n_rats in tqdm(eval_dataloader, total=len(eval_dataloader)):
            model.eval()
            q_a_input_ids, q_a_input_mask, q_a_segment_ids, q_a_img_feats = tuple(t.to(args.device) for t in batch_q_a[:-3])
            q_a_q_ids, q_a_labels = batch_q_a[-3], batch_q_a[-2]
            ...
            with torch.no_grad():
                q_a_inputs = {'input_ids':      q_a_input_ids,
                          ...
                          'img_feats':      None if args.img_feature_dim == -1 else q_a_img_feats,
                            "head": "ans"}
                outputs = model(**q_a_inputs)[0]
                q_a_logits = 1-(outputs[:, :].softmax(-1)[:, 1].view(-1))
                ...
                qa_r_inputs = {'input_ids': qa_r_input_ids,
                              ...
                              'img_feats': None if args.img_feature_dim == -1 else qa_r_img_feats,
                               "head": "rat"}
                outputs = model(**qa_r_inputs)[0]
                qa_r_logits = 1 - (outputs[:, :].softmax(-1)[:, 1].view(-1))

                for n in range(int(len(q_a_input_ids)/n_ans)):
                    q_a_pred = q_a_logits[n*n_ans: (n+1)*n_ans].argmax()
                    q_a_pred_logits = q_a_logits[n*n_ans: (n+1)*n_ans]
                    ans_label = q_a_labels[n*n_ans]

                    if n_rats > n_ans:
                        qa_r_pred_logits = qa_r_logits[n * n_rats: (n + 1) * n_rats][ans_label*n_ans: (ans_label+1)*n_ans]
                        qa_r_pred = qa_r_pred_logits.argmax()
                    else:
                        qa_r_pred_logits = qa_r_logits[n * n_rats: (n + 1) * n_rats]
                        qa_r_pred = qa_r_pred_logits.argmax()
                    rat_label = qa_r_labels[n * n_rats]

                    result = {}
                    result['questionId'] = q_a_q_ids[n*n_ans]
                    result["answer"] = [int(q_a_pred), int(qa_r_pred)]
                    result["correct"] = bool((ans_label==q_a_pred)&(rat_label==qa_r_pred))
                    result["gt"] = [ans_label, rat_label]
                    result["logits"] = [q_a_pred_logits.cpu().numpy(), qa_r_pred_logits.cpu().numpy()]
...
def main():
    ...
    # result, score = evaluate(args, model, eval_dataset, prefix="", tokenizer=tokenizer)
