...
from oscar.datasets.refcoco_ft_dataset import NormalFinetuneDataset
from oscar.modeling.modeling_rec import REC_FT
...
def collate(batch):
    ...
    img_keys, img_feat_list, input_ids_list, input_mask_list, segment_ids_list, answer_tensor_list = new_batch
    ...
    return img_keys, (img_feat_list, input_ids_list, input_mask_list, segment_ids_list, answer_tensor_list)
...
def make_data_sampler(dataset, shuffle, distributed):
    if distributed:
        return torch.utils.data.distributed.DistributedSampler(dataset, shuffle=shuffle, rank=get_rank(), num_replicas=get_world_size())
    if shuffle:
...
def eval(args, model, data_loader, tokenizer):
    model.eval()
    preds = {}
    pred_distrib = {}
    for step, (img_keys, batch) in enumerate(data_loader):
        answer_tensor_list = [t.to(args.device) for t in batch[-1]]
        batch = tuple(t.to(args.device) for t in batch[:-1])
        img_feat, input_ids, input_mask, segment_ids = batch
        # eval
        with torch.no_grad():
            loss, logits, pred_idxs = model(input_ids, segment_ids, input_mask, img_feats=img_feat, labels=answer_tensor_list)
        for p in pred_idxs:
            pred_distrib[int(p)] = pred_distrib.get(int(p), 0) + 1

        acc = [bool(gt[idx]>0.5) for idx, gt in zip(pred_idxs, answer_tensor_list)]
        for rst, imkey in zip(acc, img_keys):
            preds[imkey] = rst

    pred_list = all_gather(preds)
    for cur_preds in pred_list:
        for k, v in cur_preds.items():
            assert (k not in preds) or (preds[k]==v)
            preds[k] = v

    pred_distrib_list = all_gather(pred_distrib)
    pred_dist = {}
    for dic in pred_distrib_list:
        for k,  v in dic.items():
            pred_dist[k] = pred_dist.get(k, 0) + v
    assert len(preds) == len(data_loader.dataset)
    logger.info("acc: {:.4f} ({}/{})".format( sum(preds.values())/len(preds), sum(preds.values()), len(preds) ))
    if is_main_process():
        logger.info(str(pred_dist))
    return sum(preds.values())/len(preds)


def train_batch(args, train_loader, test_loader, model, optimizer, tokenizer, config, global_step=0, best_acc=0):
...
def main():
    ...
    config_class, model_class, tokenizer_class = BertConfig, REC_FT, BertTokenizer
    checkpoint = args.eval_model_dir
    ...
    best_acc = 0
    global_step = 0
    config.num_train_steps = args.num_train_epochs*10
    config.warmup_steps = int(config.num_train_steps / 10)
    for epoch in range(config.num_train_steps):
        logger.info("epoch: {}/{}".format(epoch+1, config.num_train_steps))
        global_step, best_acc = train_batch(args, train_loader, test_loader, model, optimizer, tokenizer, config, global_step=global_step, best_acc=best_acc)
        # if (epoch+1) % 10 == 0:
        #     acc= eval(args, model, test_loader, tokenizer)
