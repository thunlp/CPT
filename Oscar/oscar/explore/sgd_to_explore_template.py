...
from oscar.datasets.mlm_dataset import CheatTemplateTSVDataset
import pickle
import json
...
class TemplateEmbedding(nn.Module):
    def __init__(self, bert_embedding):
        ...
        self.template_embeddings = nn.Embedding(21, bert_embedding.word_embeddings.embedding_dim)\
            .to(bert_embedding.word_embeddings.weight.device)
        self.template_embeddings.weight.data[1:] = bert_embedding.word_embeddings.weight[1:21].data
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        mask1 = (0 < new_input_ids) & (new_input_ids < 21)
        new_input_ids[~mask1] = 0

        template_embeddings = self.template_embeddings(new_input_ids)
        words_embeddings = torch.where(new_input_ids.unsqueeze(-1) > 0, template_embeddings, words_embeddings)
...
def collate(batch):
    ...
    img_keys, img_feat_list, input_ids_list, input_mask_list, segment_ids_list, mask_token_pos, colors \
    ...
    return img_keys, (img_feat_list, input_ids_list, input_mask_list, segment_ids_list, mask_token_pos, colors)
...
def build_dataset(data_file, tokenizer, args):
    ...
    return CheatTemplateTSVDataset(data_file, tokenizer=tokenizer, args=args)
...
def main():
    ...
    model.eval()
    model.bert.embeddings = TemplateEmbedding(model.bert.embeddings)
    # optimizer
    tokenizer.add_special_tokens({ "additional_special_tokens": [ "[unused{}]".format(i) for i in range(21)] })
    optimizer = optim.AdamW([{"params": model.bert.embeddings.template_embeddings.weight}], lr=0.02, eps=1e-8)
    ...
    for step, (img_keys, batch) in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        # train
        optimizer.zero_grad()
        x = model(input_ids_list, segment_ids_list, input_mask_list, img_feats=img_feat_list,
                  masked_lm_labels=mlm_labels, next_sentence_label=0)

        loss = x[0]
        loss /= len(input_ids_list)

        loss.backward()
        optimizer.step()

        if step >0 and step % 10 == 0:
            acc = (x[1][torch.arange(mask_token_pos.size(0)), mask_token_pos, :].argmax(1)) == colors
            acc = acc.sum().float()/len(colors)
            print("loss: {:.4}    acc: {:.4}".format(float(loss), float(acc)))
            del acc
            preds = model.cls.predictions(model.bert.embeddings.template_embeddings.weight.data)
            vals, inds = preds.sort(1, descending=True)
            l = []
            for ids, vs in zip(inds, vals):
                tokens = tokenizer.convert_ids_to_tokens(ids[:20].tolist())
                vs = vs[:20].tolist()
                l.append(list(zip(tokens, vs)))
            pickle.dump(l, open("tmp/template.pk", "wb"))
    import IPython
    IPython.embed()
