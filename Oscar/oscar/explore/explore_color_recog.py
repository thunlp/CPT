...
def build_dataset(data_file, tokenizer, args):
    ...
    return RefCOCOTSVDataset(data_file, tokenizer=tokenizer, args=args)
...
def main():
    ...
    for step, (img_keys, batch) in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        for im_feats, input_ids, segment_ids, input_mask, mask_pos, color_set, rect in \
            ...
            with torch.no_grad():
                color_names = ["red", "purple", "green", "yellow", "blue"]
                color_ids = tokenizer.convert_tokens_to_ids(color_names)
                ...
                pred_dic[color_names[max_idx]] = pred_dic.get(color_names[max_idx], 0) + 1
                gt_dic[color_set[0]] = gt_dic.get(color_set[0], 0) + 1
                if color_names[max_idx] == color_set[0]:
                    acc_dic[color_set[0]] = acc_dic.get(color_set[0], 0) + 1
                    acc += 1
    print(acc/n, acc, n)
    print(pred_dic)
    print(gt_dic)
    print(acc_dic)
    print({k: round(v*100/gt_dic[k], 2)for k, v in acc_dic.items()})
