...
class ZSLVGDataset(Dataset):
    ...
    def __getitem__(self, item):
        # load data
        img_name, od_labels, im_feat, subj_obj_names, colors = self.decode_features(self.corpus_tsvfile, item)
        ...
        caption = subj_obj_names[0] + " is {} a " + subj_obj_names[1]
        re_caption = subj_obj_names[1] + " is {} a " + subj_obj_names[0]
        captions = [caption.format(" [MASK]"*(i+1)) for i in range(3)] + \
                   [re_caption.format(" [MASK]"*(i+1)) for i in range(3)]
        ...
        for i, caption in enumerate(captions):
            ...
            mask_token_pos.append((input_ids == 103).nonzero(as_tuple=False).squeeze())
        ...
        im_feat = torch.stack([im_feat]*6, 0)
        return img_name, im_feat, input_ids_list, input_mask_list, segment_ids_list, mask_token_pos, colors

    def decode_features(self, feat_tsv, img_idx):
        img_name, feat_str = feat_tsv.seek(img_idx)
        feat_info = json.loads(feat_str)
        objs, colors, names, _ = feat_info["objects"]
        # decode features
        im_feats = None
        od_labels = None
        ...
        return img_name, od_labels, im_feats, names, colors
