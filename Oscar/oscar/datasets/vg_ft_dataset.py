...
class FtVGNormalDataset(Dataset):
    ...
    def __getitem__(self, item):
        # load data
        img_name, od_labels, im_feat, _, rel_labels, rel_pairs = self.decode_features(self.corpus_tsvfile, item)
        ...
        return img_name, im_feat, input_ids, input_mask, segment_ids, rel_pairs, rel_labels

    def decode_features(self, feat_tsv, img_idx):
        img_name, feat_str = feat_tsv.seek(img_idx)
        feat_info = json.loads(feat_str)
        objs, colors, rel_labels, rel_pairs = feat_info["objects"]
        ...
        return img_name, od_labels, im_feats, None, rel_labels, rel_pairs
