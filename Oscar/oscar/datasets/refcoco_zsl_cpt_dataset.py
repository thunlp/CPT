...
def template1(caption, posi_tokens):
    return caption + " is [MASK]."


def template2(caption, posi_tokens):
    return caption + " is [MASK] color."


def template3(caption, posi_tokens):
    return caption + " is in [MASK] color."


def template4(caption, posi_tokens):
    posi_token = posi_tokens[-1]
    if posi_token == len(caption):
        ret = caption[: posi_token] + " in [MASK]."
    else:
        ret = caption[: posi_token] + " in [MASK]" + caption[posi_token:] + "."
    return ret


def template5(caption, posi_tokens):
    posi_token = posi_tokens[-1]
    if posi_token == len(caption):
        ret = caption[: posi_token] + " in [MASK] color."
    else:
        ret = caption[: posi_token] + " in [MASK] color" + caption[posi_token:] + "."
    return ret


def template6(caption, posi_tokens):
    posi_token = posi_tokens[0]
    ret = caption[: posi_token] + "[MASK] " + caption[posi_token:] + "."
    return ret


tmp_list = {1: template1, 2: template2, 3: template3, 4:template4, 5:template5, 6:template6}


class ZSLColorFinetuneDataset(Dataset):
    def __init__(self, data_file, args=None, tokenizer=None, txt_seq_len=70, img_seq_len=50, is_train=False, **kwargs):
        ...
        self.datasest_name =  os.path.basename(os.path.dirname(data_file))
        ...
        self.template = tmp_list[args.template]
    ...
    def __getitem__(self, item):
        ...
        # posi_token = self.anns_dic[str(img_name)]['tokens_positive'][-1]
        posi_token = 0
        ...
        for i, (od_labels, img_feat) in enumerate(zip(od_labels_list, img_feat_list)):
            to_use_caption = self.template(caption, posi_token)
        ...
        if self.is_train:
            ...
            if len(posids) == 0:
                posids = [0]
            if len(posids) > 1 and len(self) > 8:
