...
class GQAProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s-%s" % (set_type, str(i))
            text_a = line['q']
            text_b = line.get('o', None) # or None
            label = line.get('an', None)
            score = 0
            img_key = line['img_id']
            q_id = int(line['q_id'])
            examples.append(InputInstance(guid=guid, text_a=text_a, text_b=text_b, label=label, score=score, img_key=img_key, q_id=q_id))
...
class VCR_Q_A_Processor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s-%s" % (set_type, str(i))
            text_a = line['question'] # question
            # text_a = _vcr2txt(text_a)
            choices = line['answers']
            # choices = [_vcr2txt(c) for c in choices]
            label = None if set_type.startswith('test') else line['answer_label']
            img_key = line['annot_id'] # img_key = line['img_id']
            q_id = int(line['annot_id'].split('-')[-1]) #int(line['q_id']) if set_type.startswith('test') else 0
            examples.append(InputInstance(guid=guid, text_a=text_a, text_b=choices, label=label, img_key=img_key, q_id=q_id))
        return examples


def _vcr2txt(txt_l):
    person2txt = lambda pl: "person {}".format(", ".join([str(p) for p in pl]))
    txt_l = [person2txt(x) if type(x) is list else x for x in txt_l]
    return " ".join(txt_l)


def _colorize_line(line):
    to_mark, colors = _construct_to_mark_and_color(line)
    line["question"] = _vcr2colortxt(line["question"], to_mark, colors)
    line["answers"] = [_vcr2colortxt(x, to_mark, colors) for x in line["answers"]]
    line["rationales"] = [_vcr2colortxt(x, to_mark, colors) for x in line["rationales"]]
    return line


def _vcr2colortxt(txt_l, to_mark, colors):
    person2txt = lambda pl: "person {}".format(", ".join([str(p) for p in pl]))
    color_txt_l = []
    lst2str = lambda x: "_".join([str(y) for y in sorted(x)])
    for word in txt_l:
        if type(word) is list:
            if lst2str(word) in to_mark:
                c = colors[to_mark.index(lst2str(word))]
                # color_txt_l.append("the {}".format(c))
                color_txt_l.append("person" if len(word)==1 else "people")
                # color_txt_l.append("in
                # {} color".format(c))
            else:
                color_txt_l.append("person" if len(word)==1 else "people")
                # color_txt_l.append(person2txt(word))
        else:
            color_txt_l.append(word)
    return color_txt_l


def _construct_to_mark_and_color(ann):
...
class VCR_QA_R_Processor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s-%s" % (set_type, str(i))
            text_a = line['question'] +  line['answers'][line['answer_label']] # question_choice
            choices = line['rationales'] # rational_choice
            label = None if set_type.startswith('test') else line['rationale_label'] # rational_label
            img_key = line['annot_id'] # img_key = line['img_id']
            q_id = int(line['annot_id'].split('-')[-1]) #int(line['q_id']) if set_type.startswith('test') else 0
...
class VCR_QAR_Processor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # if set_type!='test': continue
            guid = "%s-%s" % (set_type, str(i))
            text_a = line['question']  # question_choice
            ...
            q_id = int(line['annot_id'].split('-')[-1])  # int(line['q_id']) if set_type.startswith('test') else 0
            examples.append(
                InputInstance(guid=guid, text_a=text_a, text_b=choices, label=label, score=None, img_key=img_key,
                              q_id=q_id))
        return examples
    ...
    #         for ans in answers:
    #             for rat in rations:
    #                 choices.append(ans+rat)
    #
    #         label = None if set_type.startswith('test') else line['answer_label']*4+line['rationale_label']  # rational_label
    #
    #         img_key = line['annot_id']  # img_key = line['img_id']
    #         q_id = int(line['annot_id'].split('-')[-1])  # int(line['q_id']) if set_type.startswith('test') else 0
    #         examples.append(
