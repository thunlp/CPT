...
def eval(args, model, data_loader, tokenizer):
    ...
    for step, (img_keys, batch) in enumerate(data_loader):
        ...
        for im_feats, input_ids, segment_ids, input_mask, mask_pos, color_set, rect in \
            ...
            mask_pos = torch.tensor(mask_pos).to(args.device)
            with torch.no_grad():
                color_ids = tokenizer.convert_tokens_to_ids(color_set)
                x = model(input_ids, segment_ids, input_mask, img_feats=im_feats)
                score = x[0][0, mask_pos].mean(0)
                score = score[color_ids]
                # assert score[0] > 0
                if len(score_list) == 0:
                    score_list = score.cpu().tolist()
                else:
                    anchor_score = score_list[0]
                    score = score * (anchor_score / score[0])
                    score_list += score.cpu().tolist()[1:]
        assert len(score_list) == len(rect_list)
        max_idx = np.argmax(score_list)
        max_rect = rect_list[max_idx]
        dic[img_keys[0]] = [max_rect, int(max_idx), score_list, rect_list]

    json.dump(dic, open("tmp/rect{}.json".format(get_rank()), "w"))
    synchronize()
    miou = None
    if is_main_process():
        rect = {}
        for i in range(args.num_gpus):
            rect.update(json.load(open("tmp/rect{}.json".format(i))))
            os.remove("tmp/rect{}.json".format(i))

        preds = rect
        gts = json.load(open("/data_local/zhangao/codes/prompt_feat/data/refcoco/split/finetune_refcoco+_testA.json"))
        gts = {str(g['id']): g['bbox'] for g in gts}

        # gts = { k: [v[0], v[1], v[0]+v[2]-1, v[1]+v[3]-1] for k, v in gts.items()}
        ...
        for k, p in preds.items():
            ...
            miou += iou > 0.5
        logger.info("miou: {:.2f}".format(miou/len(preds)*100))
    synchronize()
    return miou
...
def train_batch(args, config, train_loader, test_loader, model, optimizer, tokenizer, global_step=0):
    size = {}
    for step, (img_keys, batch) in tqdm(enumerate(train_loader), total=len(train_loader)):
        model.train()
        batch = tuple(t.to(args.device) for t in batch)
        img_feat_list, input_ids_list, input_mask_list, segment_ids_list, mask_token_pos, colors = batch
        idx = int(img_feat_list.size(0))
        size[idx] = size.get(idx, 0) + 1
    print(size)
