...
from maskrcnn_benchmark.engine.inference_vcr import inference
...
def run_test(cfg, model, distributed, model_name):
    ...
    data_loaders_val = make_data_loader(cfg, is_train=False, is_distributed=distributed, is_vg=True)
    # labelmap_file = config_dataset_file(cfg.DATA_DIR, cfg.DATASETS.LABELMAP_FILE)
    labelmap_file = "VG-SGG-dicts-vgoi6-clipped.json"
    ...
    if not cfg.MODEL.RPN_ONLY and cfg.MODEL.ATTRIBUTE_ON and (not cfg.TEST.SKIP_PERFORMANCE_EVAL):
        data_loaders_val = make_data_loader(cfg, is_train=False, is_distributed=distributed, is_refcoco=True)
...
def main():
    ...
    data_split_name = load_from_yaml_file(cfg.DATA_DIR)['ann']
    data_split_name = os.path.basename(data_split_name).replace(".json", "")
    data_split_name = data_split_name.replace("finetune_", "")
    run_test(cfg, model, args.distributed, data_split_name)
