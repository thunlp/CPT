...
def draw_seg(img, mask, box=None, color=(240, 0, 30, 70)):
    ...
    if box:
        ...
        # foreground = Image.new('RGBA', (int(x2 - x1), int(y2 - y1)), color=color)
        # img.paste(foreground, (int(x1), int(y1)), foreground)
        img = img.crop((x1, y1, x2, y2))
    return img

images = {}
for x in data:
    if get_img_id(x['file_name']) not in refcoco_dic:
        continue
    fname = x['file_name']
