...
from .collate_batch import BatchCollator, BBoxAugCollator, RefCOCOCollator, VGCollator
...
def build_dataset(cfg, transforms, dataset_catalog, is_train=True):
    ...
    for i, dataset_name in enumerate(dataset_list):
        ...
        args["yaml_file"] = cfg.DATA_DIR
        factory = getattr(D, dataset_name)
...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0, is_for_period=False, is_refcoco=False, is_vg=False):
    ...
    for dataset in datasets:
        ...
        if not is_refcoco and not is_vg:
            collator = BBoxAugCollator() if not is_train and cfg.TEST.BBOX_AUG.ENABLED else \
                BatchCollator(cfg.DATALOADER.SIZE_DIVISIBILITY)
        elif is_refcoco:
            collator = RefCOCOCollator(cfg.DATALOADER.SIZE_DIVISIBILITY)
        elif is_vg:
            collator = VGCollator(cfg.DATALOADER.SIZE_DIVISIBILITY)
