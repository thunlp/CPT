...
class GQAColorDataset(object):
    def __init__(self, yaml_file, transforms=None, **kwargs):
        ...
        self.anns = [ann for ann in self.anns if str(ann['q_id']) in self.to_mark]
    ...
    def __getitem__(self, idx):
        ...
        if len(to_mark) > 0:
            self.draw_rectangles(img, BoxList(to_mark, img_size, mode="xyxy"), self.colors, save_name=str(ann["q_id"]))
    ...
    def construct_to_mark(self, ann, img_size,):
        ...
        def recover_box(box, img_size):
            ...
            return box
        # construct dets
        to_mark = self.to_mark[str(q_id)] # [    [[75.0, 359.0, 105.0, 444.0], 4]...    ]
