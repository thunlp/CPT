...
def compute_on_dataset(model, data_loader, device, bbox_aug, timer=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        images, targets, captions, color_names, rects, image_ids = batch[0], batch[1], batch[2], batch[3], batch[4], batch[5]
        ...
        with torch.no_grad():
            ...
            if bbox_aug:
                ...
                try:
                    output = []
                    n_batch = 64
                    for i in range(math.ceil(len(targets)/n_batch)):
                        output = output + model(images[i*n_batch: (i+1)*n_batch].to(device), targets[i*n_batch: (i+1)*n_batch])
        ...
        # assign output to different bags
        dets = []
        n = 0
        for i, cname_set in enumerate(color_names):
            dets.append(output[n: n+len(cname_set)])
            n += len(cname_set)
        assert n == len(output)
        results_dict.update(
            {img_id: [result, cap, cname_set, rect_set]
             for img_id, result, cap, cname_set, rect_set in zip(image_ids, dets, captions, color_names, rects)}
        )
    return results_dict
...
def convert_predictions_to_tsv(predictions, dataset, output_folder,
    ...
    def gen_rows():
        for idx, prediction in sorted(predictions.items()):
            ...
            to_save_preds = []
            for p in preds:
                p = p.resize((image_width, image_height))
                boxes = p.bbox.tolist()
                ...
                if 'feature' in data_subset:
                    features = p.get_field('box_features').numpy()
                if 'scores_all' in data_subset:
                    scores_all = p.get_field('scores_all').numpy()
                if 'boxes_all' in data_subset:
                    boxes_all = p.get_field('boxes_all').numpy()
                if "attr_labels" in data_subset:
                    attr_labels = p.get_field("attr_labels").tolist()
                if "attr_scores" in data_subset:
                    attr_scores = p.get_field("attr_scores").tolist()
                if "attr_scores_all" in data_subset:
                    attr_scores_all = p.get_field("attr_scores_all").numpy()
                ...
                to_save_preds.append(objects)
            prediction[0] = to_save_preds
            yield image_key, json.dumps({'objects': prediction})
...
def inference(
    ...
    # modify features
    for img_key, boxlist_list in predictions.items():
        boxlist_list = boxlist_list[0]
        for boxlist in boxlist_list:
            w, h = boxlist.size
            features = boxlist.get_field("box_features")
            boxes = boxlist.bbox
            norm_mask = torch.tensor([w, h, w, h]).to(boxlist.bbox.device)
            box_feats = boxes / norm_mask
            additional_feats = torch.stack((box_feats[:, 2] - box_feats[:, 0], box_feats[:, 3] - box_feats[:, 1])).T
            box_feats = torch.cat([box_feats, additional_feats], 1)
            features = torch.cat([features, box_feats], 1)
            boxlist.extra_fields['box_features'] = features
    ...
    #     cur_predictions = {data_loader.dataset.get_img_key(k): v for k, v in predictions.items()}
    #     torch.save(cur_predictions, os.path.join(output_folder, output_pth_name))
