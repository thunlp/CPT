...
def compute_on_dataset(model, data_loader, device, bbox_aug, timer=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        images, targets,  color_names, label_names, image_ids, rel_names = batch[0], batch[1], batch[2], \
        ...
        results_dict.update(
            {img_id: [result, cname, lb_name, rel_lb] for img_id, result, cname, lb_name, rel_lb
             in zip(image_ids, output, color_names, label_names, rel_names)}
        )
    return results_dict
...
def inference(
    ...
    # modify features
    for img_key, boxlist in predictions.items():
        boxlist = boxlist[0]
        w, h = boxlist.size
