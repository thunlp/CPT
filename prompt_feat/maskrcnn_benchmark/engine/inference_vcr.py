...
def compute_on_dataset(model, data_loader, device, bbox_aug, timer=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        images, targets, image_ids, scales,  txt_colors, txt_names = batch[0], batch[1], batch[2], \
        ...
        results_dict.update(
            {img_id: [result, t_color, t_name] for img_id, result, t_color, t_name in zip(image_ids, output, txt_colors, txt_names)}
        )
    return results_dict
...
def convert_predictions_to_tsv(predictions, dataset, output_folder,
    ...
    def gen_rows():
        for idx, raw_prediction in sorted(predictions.items()):
            ...
            yield image_key, json.dumps({'objects': [objects, raw_prediction[1:]], 'relations': triplets})
